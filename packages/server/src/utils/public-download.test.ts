...
function mockResponse(statusCode: number, headers: IncomingHttpHeaders = {}, body = ''): IncomingMessage {
  const stream = Readable.from(body ? [Buffer.from(body)] : []);
  const response = stream as unknown as IncomingMessage;
  (response as { statusCode: number }).statusCode = statusCode;
  (response as { headers: IncomingHttpHeaders }).headers = headers;
  return response;
}
...
test('downloadPublicUrl blocks private DNS resolution', async () => {
  await assert.rejects(
    () =>
      downloadPublicUrl('http://example.com/file.txt', {
        maxBytes: 1024,
        timeoutMs: 500,
        maxRedirects: 1,
    ...
    () =>
      downloadPublicUrl('http://example.com/file.txt', {
        ...
        _lookupFn: async (hostname) => {
          ...
          if (hostname === 'internal.local') return [{ address: '10.0.0.20', family: 4 }];
          return [];
        },
        _requestFn: async () => {
          requestCount += 1;
