...
function isValidPgIdentifier(value: string): boolean {
...
const envSchema = z
  ...
  .superRefine((values, ctx) => {
    ...
    if (
      !Number.isFinite(values.EVALUATION_QUEUE_RECOVERY_MIN_PENDING_AGE_MS) ||
    ...
    if (
      !Number.isInteger(values.EVALUATION_QUEUE_RECOVERY_RUN_BATCH_SIZE) ||
    ...
    if (
      !Number.isInteger(values.EVALUATION_QUEUE_RECOVERY_IMPORT_BATCH_SIZE) ||
