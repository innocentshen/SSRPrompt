...
type PromptMessage = { role: PromptMessageRole; content: string };
...
type CompletionMessage =
  | { role: PromptMessageRole; content: string }
  | { role: PromptMessageRole; content: Array<{ type: 'text'; text: string } | AttachmentPart> };
...
function containsInputPlaceholder(text: string): boolean {
  return /\{\{\s*input\s*\}\}/.test(text);
}

function renderTemplateText(template: string, values: Record<string, string>): string {
...
function parseMessagesFromContent(content: string | null | undefined): PromptMessage[] {
  ...
  try {
    ...
    const messages = parsed
      ...
      .map((item) => ({ role: item.role as PromptMessageRole, content: item.content as string }));
...
function parsePromptMessages(messages: unknown, content: string | null): PromptMessage[] {
...
function parsePromptVariables(variables: unknown): Array<{ name: string; default_value?: string }> {
...
function toResponseFormat(outputSchema: unknown): Record<string, unknown> | undefined {
  ...
  const jsonSchema: Record<string, unknown> = { name, schema };
...
function resolveReasoning(value: unknown):
  | { enabled: boolean; effort?: 'default' | 'none' | 'low' | 'medium' | 'high' }
  | undefined {
  if (!isRecord(value) || typeof value.enabled !== 'boolean') return undefined;

  const effort = value.effort;
  if (effort === undefined) return { enabled: value.enabled };
...
function extractLastUserMessage(metadata: unknown, fallbackInput: string): string | null {
...
function mergeConversationHistory(
  currentMessages: CompletionMessage[],
  turns: ConversationTurn[]
): CompletionMessage[] {
  if (turns.length === 0) return currentMessages;
...
async function resolvePromptSnapshot(
  userId: string,
  promptId: string,
  requestedVersion: z.infer<typeof VersionSelectorSchema> | undefined
  ...
  let targetVersion: number | 'latest' = 'latest';
...
function buildMessagesFromPrompt(snapshot: PromptSnapshot, payload: z.infer<typeof InvokePromptSchema>): CompletionMessage[] {
  ...
  const renderedMessages: CompletionMessage[] =
  ...
  if (shouldAppendInputText || attachmentParts.length > 0) {
    if (attachmentParts.length === 0) {
      ...
      const contentParts: Array<{ type: 'text'; text: string } | AttachmentPart> = [];
