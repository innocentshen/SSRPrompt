...
import type { PromptApiKey as PrismaPromptApiKey } from '@prisma/client';
import {
  AppError,
  type CreatePromptApiKeyInput,
  type CreatePromptApiKeyResult,
  type PromptApiKey,
...
function ensureFutureDate(date: Date | null): void {
  if (!date) return;
...
function hashPromptApiKey(apiKey: string): string {
  return createHash('sha256')
    .update(`${env.JWT_SECRET}:${apiKey.trim()}`)
    .digest('hex');
}

function generatePromptApiKeyPlain(): string {
  return `${API_KEY_PREFIX}${randomBytes(24).toString('base64url')}`;
}

function mapPromptApiKey(row: PrismaPromptApiKey): PromptApiKey {
  return {
    id: row.id,
    userId: row.userId,
    name: row.name,
...
export class PromptApiKeysService {
  async list(userId: string): Promise<PromptApiKey[]> {
  ...
  async create(userId: string, input: CreatePromptApiKeyInput): Promise<CreatePromptApiKeyResult> {
  ...
  async revoke(userId: string, id: string): Promise<PromptApiKey> {
  ...
  async authenticate(apiKeyRaw: string): Promise<PromptApiKeyAuthResult> {
