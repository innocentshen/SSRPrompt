import { z } from 'zod';
import { getModelWithProvider, streamChatCompletion, chatCompletion, } from '../services/chat.service.js';
...
function normalizeStructuredJsonOutput(content, responseFormat) {
...
const CHAT_ATTACHMENT_MAX_BYTES = Math.max(1, Number(process.env.CHAT_ATTACHMENT_MAX_BYTES || String(20 * 1024 * 1024)));
async function resolveModelMaxContextLength(_userId, _model, _provider, _origin) {
...
function extractInputContent(messages) {
            ...
            if (part.type === 'image_url') {
                ...
                if (dataUrl) {
                    ...
                    if (buffer.length > CHAT_ATTACHMENT_MAX_BYTES) {
                        throw new AppError(413, 'VALIDATION_ERROR', `Attachment exceeds size limit (${CHAT_ATTACHMENT_MAX_BYTES} bytes)`);
                    }
...
export const chatController = {
    ...
    async completions(req, res) {
        ...
        const traceSource = req.promptApiKeyId ? 'api' : 'feature';
        // Get model with decrypted API key
        const { model, provider, apiKey } = await getModelWithProvider(userId, data.modelId);
        const maxContextLength = await resolveModelMaxContextLength(userId, model, provider, req.headers.origin);
        const requestedFileProcessing = data.fileProcessing ?? 'auto';
        const messagesForModel = requestedFileProcessing === 'none'
            ? stripFileParts(data.messages)
            : data.messages;
        const wantsFiles = hasFileParts(messagesForModel);
        let mode = 'vision';
        if (wantsFiles) {
            if (requestedFileProcessing === 'ocr')
                mode = 'ocr';
            else if (requestedFileProcessing === 'vision')
                mode = 'vision';
            else
        ...
        if (wantsFiles && mode === 'vision' && !model.supportsVision) {
            throw new AppError(400, 'FILE_UPLOAD_NOT_ALLOWED', 'This model does not support direct file inputs. Enable OCR to use attachments.');
        }
        ...
        const modelParameters = Object.fromEntries(Object.entries({
            temperature: data.temperature,
            top_p: data.top_p,
            max_tokens: data.max_tokens,
            frequency_penalty: data.frequency_penalty,
        ...
        if (data.stream) {
            ...
            try {
                const stream = streamChatCompletion(provider, model, apiKey, expanded.messages, options, abortController.signal);
                for await (const chunk of stream) {
                    ...
                    const maybeDetails = chunk.choices?.[0]?.message?.reasoning_details;
                ...
                if (error.name === 'AbortError') {
                    ...
                    const appError = error instanceof AppError ? error : new AppError(500, 'PROVIDER_ERROR', error.message);
                ...
                const normalizedOutput = isJsonSchemaResponseFormat(data.responseFormat)
                    ...
                    : fullContent;
                const rawThinking = fullThinking.trim() ||
                ...
                const thinkingTimeMs = thinkingStartedAt !== null && thinkingEndedAt !== null ? Math.max(0, thinkingEndedAt - thinkingStartedAt) : null;
            ...
            try {
                const result = await chatCompletion(provider, model, apiKey, expanded.messages, options);
                const latencyMs = Date.now() - startTime;
                const normalizedOutput = isJsonSchemaResponseFormat(data.responseFormat)
