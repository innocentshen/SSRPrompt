...
async function resolvePublicHostname(hostname, lookupFn) {
...
async function readIncomingMessageToBuffer(response, maxBytes) {
...
async function requestWithPinnedAddress(url, resolvedHost, timeoutMs) {
...
export async function downloadPublicUrl(urlValue, options) {
    let current;
    try {
    ...
    const lookupFn = options._lookupFn ?? ((hostname) => lookup(hostname, { all: true, verbatim: true }));
...
//# sourceMappingURL=public-download.js.map