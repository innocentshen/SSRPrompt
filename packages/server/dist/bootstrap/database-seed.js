...
const REQUIRED_ROLE_NAMES = ['admin', 'user', 'viewer'];
...
async function hasSeededTables(prisma) {
    const rows = await prisma.$queryRaw `SELECT to_regclass('public.roles')::text AS table_name`;
    return Boolean(rows[0]?.table_name);
}
async function shouldRunBootstrapSeed(prisma) {
...
export async function seedBootstrapData(prisma, options = {}) {
...
export async function autoSeedBootstrapData(prisma, options = {}) {
    const logger = options.logger ?? console;
