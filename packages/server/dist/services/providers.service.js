...
import { AppError, ForbiddenError } from '@ssrprompt/shared';
...
function buildOpenAICompatibleModelsUrl(baseUrl) {
...
async function parseErrorMessage(response) {
    ...
    if (!text)
        return `Provider API error: ${response.statusText}`;
    try {
...
export class ProvidersService {
    ...
    async update(userId, id, data, isAdmin = false) {
        ...
        if (provider.isSystem && !isAdmin) {
            ...
            const baseUrlChanged = typeof data.baseUrl !== 'undefined' && (data.baseUrl ?? null) !== (provider.baseUrl ?? null);
        ...
        if (isAdmin && typeof data.isSystem === 'boolean' && data.isSystem !== provider.isSystem) {
            ...
            return providersRepository.update(userId, id, {
                ...data,
     ...
     */
    async discoverModels(userId, providerId, input, requestOrigin) {
        ...
        const isLikelyMasked = rawApiKey === '***decryption-failed***' || (rawApiKey.endsWith('...') && rawApiKey.length <= 20);
        ...
        const effectiveBaseUrl = baseUrlOverride === undefined ? provider.baseUrl : baseUrlOverride;
        let url;
        const headers = {
            'Content-Type': 'application/json',
        };
        ...
        if (effectiveType === 'gemini') {
            return (data.models || [])
                ...
                const maxContextLength = typeof model.inputTokenLimit === 'number'
        ...
        return (data.data || [])
            .map((m) => {
            const model = m;
            const maxContextLength = typeof model.context_length === 'number'
