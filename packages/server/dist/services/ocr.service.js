...
function normalizeMineruZipUrl(value) {
    try {
...
const PADDLE_VL_DEFAULT_PARAMS = {
    useDocOrientationClassify: null,
    useDocUnwarping: null,
...
function normalizeMineruParams(_userId, raw) {
    const defaults = MINERU_DEFAULT_PARAMS;
    if (!raw || typeof raw !== 'object')
        return defaults;
    const record = raw;
    const mineruRaw = record.mineru;
    if (!mineruRaw || typeof mineruRaw !== 'object')
        return defaults;
    const mineru = mineruRaw;
    const userToken = (() => {
        if (mineru.userToken === null)
            return null;
    ...
    const modelVersion = mineru.modelVersion === 'vlm' ? 'vlm' : 'pipeline';
...
function normalizeDatalabParams(raw) {
    const defaults = DATALAB_DEFAULT_PARAMS;
    if (!raw || typeof raw !== 'object')
        return defaults;
    const record = raw;
    const datalabRaw = record.datalab;
    if (!datalabRaw || typeof datalabRaw !== 'object')
        return defaults;
    const datalab = datalabRaw;
    const toNumber = (value) => typeof value === 'number' && Number.isFinite(value) && value > 0 ? Math.trunc(value) : null;
    ...
    const toMode = (value) => value === 'balanced' || value === 'accurate' || value === 'fast' ? value : defaults.mode;
    return {
...
function normalizePaddleParams(raw) {
    const defaults = PADDLE_DEFAULT_PARAMS;
    if (!raw || typeof raw !== 'object')
        return defaults;
    const record = raw;
    const paddleRaw = record.paddle;
    if (!paddleRaw || typeof paddleRaw !== 'object')
        return defaults;
    const paddle = paddleRaw;
    ...
    const toLimitType = (value) => value === 'min' || value === 'max' ? value : null;
    return {
...
function normalizePaddleVlParams(raw) {
    ...
    const toMergeMode = (value) => value === 'large' || value === 'small' || value === 'union' ? value : null;
    return {
        useDocOrientationClassify: toBoolean(paddle.useDocOrientationClassify) ?? defaults.useDocOrientationClassify,
        useDocUnwarping: toBoolean(paddle.useDocUnwarping) ?? defaults.useDocUnwarping,
...
function mergePaddleVlParams(base, override) {
    if (!override)
        return base;
    const next = { ...base };
    if (Object.prototype.hasOwnProperty.call(override, 'useDocOrientationClassify')) {
...
function mergeDatalabParams(base, override) {
    if (!override)
        return base;
    const next = { ...base };
...
function mimeToPaddleFileType(mimeType) {
...
function applyPaddleParams(target, params) {
    if (typeof params.useDocOrientationClassify === 'boolean')
        target.useDocOrientationClassify = params.useDocOrientationClassify;
    if (typeof params.useDocUnwarping === 'boolean')
        target.useDocUnwarping = params.useDocUnwarping;
    if (typeof params.useTextlineOrientation === 'boolean')
        target.useTextlineOrientation = params.useTextlineOrientation;
    if (typeof params.textDetLimitSideLen === 'number')
        target.textDetLimitSideLen = params.textDetLimitSideLen;
    if (params.textDetLimitType === 'min' || params.textDetLimitType === 'max')
        target.textDetLimitType = params.textDetLimitType;
    if (typeof params.textDetThresh === 'number')
        target.textDetThresh = params.textDetThresh;
    if (typeof params.textDetBoxThresh === 'number')
        target.textDetBoxThresh = params.textDetBoxThresh;
    if (typeof params.textDetUnclipRatio === 'number')
        target.textDetUnclipRatio = params.textDetUnclipRatio;
    if (typeof params.textRecScoreThresh === 'number')
        target.textRecScoreThresh = params.textRecScoreThresh;
    if (typeof params.visualize === 'boolean')
        target.visualize = params.visualize;
}
function applyPaddleVlParams(target, params) {
    if (typeof params.useDocOrientationClassify === 'boolean')
        target.useDocOrientationClassify = params.useDocOrientationClassify;
    if (typeof params.useDocUnwarping === 'boolean')
        target.useDocUnwarping = params.useDocUnwarping;
    if (typeof params.useLayoutDetection === 'boolean')
        target.useLayoutDetection = params.useLayoutDetection;
    if (typeof params.useChartRecognition === 'boolean')
        target.useChartRecognition = params.useChartRecognition;
    if (typeof params.layoutThreshold === 'number')
        target.layoutThreshold = params.layoutThreshold;
    if (typeof params.layoutNms === 'boolean')
        target.layoutNms = params.layoutNms;
    if (typeof params.layoutUnclipRatio === 'number')
        target.layoutUnclipRatio = params.layoutUnclipRatio;
    ...
    if (typeof params.promptLabel === 'string' && params.promptLabel.trim())
        target.promptLabel = params.promptLabel.trim();
    if (typeof params.repetitionPenalty === 'number')
        target.repetitionPenalty = params.repetitionPenalty;
    if (typeof params.temperature === 'number')
        target.temperature = params.temperature;
    if (typeof params.topP === 'number')
        target.topP = params.topP;
    if (typeof params.minPixels === 'number')
        target.minPixels = params.minPixels;
    if (typeof params.maxPixels === 'number')
        target.maxPixels = params.maxPixels;
    if (typeof params.showFormulaNumber === 'boolean')
        target.showFormulaNumber = params.showFormulaNumber;
    if (typeof params.prettifyMarkdown === 'boolean')
        target.prettifyMarkdown = params.prettifyMarkdown;
    if (typeof params.visualize === 'boolean')
        target.visualize = params.visualize;
}
function buildAuthorization(value, defaultScheme) {
    const trimmed = value.trim();
    if (!trimmed)
        return '';
...
function extractStringsDeep(value, out, depth = 0) {
    if (depth > 6)
        return;
    if (typeof value === 'string') {
        const v = value.trim();
    ...
    if (Array.isArray(value)) {
        for (const item of value)
            extractStringsDeep(item, out, depth + 1);
        return;
    }
...
function extractPaddleText(prunedResult) {
    if (!prunedResult)
        return '';
    if (typeof prunedResult === 'string')
        return prunedResult.trim();
    if (prunedResult && typeof prunedResult === 'object') {
        const record = prunedResult;
        const candidates = [
            record.rec_texts,
            record.recTexts,
            record.texts,
        ...
        for (const c of candidates) {
            if (Array.isArray(c) && c.every((x) => typeof x === 'string')) {
                return c.map((s) => s.trim()).filter(Boolean).join('\n');
            }
            if (typeof c === 'string') {
                const v = c.trim();
                if (v)
...
function extractPaddleVlText(layoutParsingResult) {
    if (!layoutParsingResult)
        return '';
    if (typeof layoutParsingResult === 'string')
        return layoutParsingResult.trim();
    if (layoutParsingResult && typeof layoutParsingResult === 'object') {
        const record = layoutParsingResult;
...
async function paddleOcrExtract(config, file, params) {
    const url = (() => {
        try {
            const u = new URL(config.baseUrl);
            const segments = u.pathname.split('/').filter(Boolean);
            if (segments.includes('ocr'))
                return u.toString();
...
async function paddleOcrVlExtract(config, file, params) {
    const url = (() => {
        try {
            const u = new URL(config.baseUrl);
            const segments = u.pathname.split('/').filter(Boolean);
            if (segments.includes('layout-parsing'))
                return u.toString();
...
async function datalabExtract(config, params, file) {
    ...
    if (typeof params.maxPages === 'number')
        form.append('max_pages', String(params.maxPages));
    if (params.pageRange)
        form.append('page_range', params.pageRange);
    ...
    if (params.extras)
        form.append('extras', params.extras);
    if (params.additionalConfig)
        form.append('additional_config', params.additionalConfig);
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 180000);
    ...
    try {
        ...
        const requestId = typeof startJson.request_id === 'string'
            ? startJson.request_id
            : (typeof startJson.requestId === 'string' ? startJson.requestId : null);
        const requestCheckUrl = typeof startJson.request_check_url === 'string'
        ...
        const checkUrl = typeof requestCheckUrl === 'string'
            ? requestCheckUrl
        ...
        while (true) {
            ...
            if (status === 'completed' || status === 'complete' || status === 'success') {
                const outputText = (() => {
                    if (typeof pollJson.markdown === 'string' && pollJson.markdown.trim())
                        return pollJson.markdown;
                    if (typeof pollJson.html === 'string' && pollJson.html.trim())
                        return pollJson.html;
                    if (typeof pollJson.json === 'string' && pollJson.json.trim())
                        return pollJson.json;
                    if (pollJson.json && typeof pollJson.json === 'object') {
                        return JSON.stringify(pollJson.json, null, 2);
                    }
                    if (typeof pollJson.chunks === 'string' && pollJson.chunks.trim())
                        return pollJson.chunks;
...
async function parseJsonOrThrow(res, providerName) {
    const text = await res.text();
    const sanitizedText = text.replace(/^\uFEFF/, '');
    try {
...
async function mineruExtract(config, params, file) {
    ...
    try {
        ...
        const sourceUrl = await getSignedUrl(s3.client, new GetObjectCommand({
            Bucket: s3.bucket,
            Key: tempObjectKey,
        }), { expiresIn: 3600 });
        ...
        if (params.pageRanges)
            body.page_ranges = params.pageRanges;
        if (Array.isArray(params.extraFormats) && params.extraFormats.length > 0)
        ...
        const submitCode = submitJson.code;
        ...
        while (true) {
            ...
            if (state === 'done') {
                ...
                const pickLargestTextFile = (suffix) => entries
            ...
            const pollCode = submitJson.code;
...
export class OcrService {
    ...
    async updateSettings(userId, data) {
        ...
        if (data.credentialSource)
            update.credentialSource = data.credentialSource;
        const existingParams = row?.providerParams ?? null;
        const nextProviderParams = existingParams && typeof existingParams === 'object' && !Array.isArray(existingParams)
            ? { ...existingParams }
            : {};
    ...
    async updateSystemSettings(data) {
        const entries = [];
        if (data.paddle)
            entries.push({ provider: 'paddle', config: data.paddle });
        if (data.paddle_vl)
            entries.push({ provider: 'paddle_vl', config: data.paddle_vl });
        if (data.paddle_vl_1_5)
            entries.push({ provider: 'paddle_vl_1_5', config: data.paddle_vl_1_5 });
        if (data.datalab)
            entries.push({ provider: 'datalab', config: data.datalab });
        if (data.mineru)
            entries.push({ provider: 'mineru', config: data.mineru });
    ...
    async test(userId, file, override) {
        ...
        try {
            if (effective.provider === 'paddle') {
                const { pages, fullText } = await paddleOcrExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, { buffer: file.buffer, mimeType: file.mimeType }, effective.paddle);
            ...
            if (effective.provider === 'paddle_vl') {
                const { pages, fullText } = await paddleOcrVlExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, { buffer: file.buffer, mimeType: file.mimeType }, effective.paddle_vl);
                const latencyMs = Date.now() - started;
                const previewText = fullText.slice(0, OCR_TEST_PREVIEW_LIMIT);
                return {
                    success: true,
            ...
            const { pages, fullText, pageCount } = await datalabExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, effective.datalab, file);
            const latencyMs = Date.now() - started;
            const previewText = fullText.slice(0, OCR_TEST_PREVIEW_LIMIT);
            return {
                success: true,
    ...
    async getResults(userId, fileIds, provider) {
    ...
    async extractForFile(userId, fileId, override) {
        ...
        try {
            ...
            let fullText = '';
            if (effective.provider === 'paddle') {
                const result = await paddleOcrExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, { buffer, mimeType }, effective.paddle);
                pages = result.pages;
                fullText = result.fullText;
            }
            else if (effective.provider === 'paddle_vl') {
                const result = await paddleOcrVlExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, { buffer, mimeType }, effective.paddle_vl);
                pages = result.pages;
                fullText = result.fullText;
            }
                ...
                const result = await datalabExtract({ baseUrl: effective.baseUrl, apiKey: effective.apiKey }, effective.datalab, { buffer, mimeType, filename });
                pages = result.pages;
                fullText = result.fullText;
            }
