...
function resolveQueueTask(runConfig) {
    if (runConfig && typeof runConfig === 'object' && !Array.isArray(runConfig)) {
        const value = runConfig.queueTask;
...
async function recoverPendingImports() {
    const threshold = new Date(Date.now() - env.EVALUATION_QUEUE_RECOVERY_MIN_PENDING_AGE_MS);
...
export function startEvaluationQueueRecoveryDaemon() {
    if (recoveryTimer)
        return;
    if (getEvaluationQueueDriver() !== 'pg')
        return;
    if (!env.EVALUATION_QUEUE_RECOVERY_ENABLED)
        return;
    recoveryTimer = setInterval(() => {
        void runRecoveryCycle();
    }, env.EVALUATION_QUEUE_RECOVERY_INTERVAL_MS);
    void runRecoveryCycle();
    console.log(`Queue recovery daemon started (interval=${env.EVALUATION_QUEUE_RECOVERY_INTERVAL_MS}ms, schema=${env.GRAPHILE_WORKER_SCHEMA}).`);
}
export async function stopEvaluationQueueRecoveryDaemon() {
    if (!recoveryTimer)
        return;
...
//# sourceMappingURL=evaluation-queue-recovery.service.js.map